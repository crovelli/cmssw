...
EcalUncalibratedRecHit::EcalUncalibratedRecHit() : 
  amplitude_(0.), amplitudeError_(0.), secondAmplitude_(amplitude_), pedestal_(0.), jitter_(0.), chi2_(10000.), flags_(0), aux_(0) {
...
EcalUncalibratedRecHit::  EcalUncalibratedRecHit(const DetId& id, float ampl, float ped,
  ...
  amplitude_(ampl), amplitudeError_(0.), secondAmplitude_(amplitude_), pedestal_(ped), jitter_(jit), chi2_(chi2), flags_(flags), aux_(aux), id_(id) {
  const unsigned int nsample = EcalDataFrame::MAXSAMPLES;
  for(unsigned int ibx=0; ibx<nsample; ++ibx) OOTamplitudes_[ibx] = 0.;  
}
