...
from Calibration.EcalCalibAlgos.DYJetsToLLSpring15_25ns_OfficialSkimZee_cff import *   
process.source = cms.Source("PoolSource",  
                            fileNames = readFilesRM,         
#                            fileNames=cms.untracked.vstring("file:zeeCalibSkim.root")
)

# Other statements
from Configuration.AlCa.GlobalTag import GlobalTag
if (not isMC):
    process.GlobalTag.globaltag = 'GR_R_74_V12'             # da cambiare
else:
...
process.ecalRecHit.EBRecalibRecHitCollection = "EcalRecHitsEB"          # chiara: si chiamano EcalRecHitsEB ma sono reduced. Da capire
                                                                        # quale formato ci sara'. Inoltre, considera che il PF dopo
process.ecalRecHit.EERecalibRecHitCollection = "EcalRecHitsEE"          # avrebbe bisogno i non reduced! che sono usati in
...
process.looper.maxLoops = cms.untracked.uint32(1)     # 7              
...
#Setting to null value avoids reading mc infos (event weight)      
#process.looper.mcProducer = cms.untracked.string('')              # uncomment to run on data
# not to be changed
