  ...
  for (int i=0;i<(int)channels_;i++) { 
	...
	if (weight_sum_[i]!=0.) {
	  ...
	  if (peak >=MIN_RESCALE && peak <= MAX_RESCALE)
	    ...
	    optimizedCoefficients_[i] = 1 / (1 + calib_fac_[i]/weight_sum_[i]);      // questo: entra qui e in getWeightSum, getEpsilonSum
...
bool ZIterativeAlgorithmWithFit::addEvent(calib::CalibElectron* ele1, calib::CalibElectron* ele2, float invMassRescFactor, float mcWeight) {   
  ...
  if((ele2->getParentSuperCluster()->position().eta() > -10.) && (ele2->getParentSuperCluster()->position().eta() < 10.) && 
    ...
    getWeight(currentEvent_, Electrons, invMassRescFactor, mcWeight);   
  }
  currentEvent_++;
  return kTRUE;
}

void ZIterativeAlgorithmWithFit::getWeight(unsigned int event_id, std::pair<calib::CalibElectron*,calib::CalibElectron*> elepair, float invMassRescFactor, float mcWeight)   
{
  getWeight(event_id, elepair.first, invMassRescFactor, mcWeight);     
  getWeight(event_id, elepair.second, invMassRescFactor, mcWeight);
}
...
void ZIterativeAlgorithmWithFit::getWeight(unsigned int event_id, calib::CalibElectron* ele, float evweight, float mcWeight) {
	...
	if (toBeConsidered)
	  {
		...
		calib_fac_[mod] += weight2 * rescale;    // questo? direi di si: weight2*mcWeight
		weight_sum_[mod]+= weight2;              // questo? direi di si: weight2*mcWeight

		thePlots_->weightedRescaleFactor[currentIteration_][mod]->Fill(rescale,weight2*mcWeight);
		thePlots_->unweightedRescaleFactor[currentIteration_][mod]->Fill(rescale,1.*mcWeight);
		thePlots_->weight[currentIteration_][mod]->Fill(weight2,1.*mcWeight);
		thePlots_->weightedZmassVsChannel[currentIteration_]->Fill(mod,massReco_[event_id],weight2*mcWeight);  
